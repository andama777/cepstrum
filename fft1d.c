/*==================================================================
  ...
  Modification: 
   ...
   (N.B)  Argument nn in ComplexFFT1D()
           n  : datasize
           nn : datanum (ComplexFFT)
...
void 
FFT1D(int nn, int isgn, Complex *cn)
{
    ...
    n = nn*2;  /* 複素数なのでデータが2倍 */
    ip = (int *)calloc(sqrt(nn) + 2, sizeof(int));
    if(ip == NULL){
      fprintf(stderr, "in ComplexFFT1D(): Can't allocate memory for ip.\n");
      exit(1);
    }
    w = (double *)calloc(nn/2, sizeof(double));
    if(w == NULL){
      fprintf(stderr, "in ComplexFFT1D(): Can't allocate memory for w.\n");
      exit(1);
    }
...
DCT1D(int n, int isgn, double *a)
{
    ...
    ip = (int *)calloc((sqrt(n/2) + 2), sizeof(int));
    if(ip == NULL){
      fprintf(stderr, "in DCT1D(): Can't allocate memory for ip.\n");
      exit(1);
    }
    w = (double *)calloc((n*5/4), sizeof(double));
    if(w == NULL){
      fprintf(stderr, "in DCT1D(): Can't allocate memory for w.\n");
      exit(1);
    }
...
DST1D(int n, int isgn, double *a)
{
    ...
    ip = (int *)calloc((sqrt(n/2) + 2), sizeof(int));
    if(ip == NULL){
      fprintf(stderr, "in DST1D(): Can't allocate memory for ip.\n");
      exit(1);
    }
    w = (double *)calloc((n*5/4), sizeof(double));
    if(w == NULL){
      fprintf(stderr, "in DST1D(): Can't allocate memory for w.\n");
      exit(1);
    }
